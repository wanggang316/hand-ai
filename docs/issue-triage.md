...
| #  | Title (short)                                    | State                  | Reproduces?      | Notes                                                                                  |
| -- | ------------------------------------------------ | ---------------------- | ---------------- | -------------------------------------------------------------------------------------- |
| 14 | fs_watch watches files directly (FSEvents)       | Partial (1/8 fixed)    | yes              | rg detection fixed in `ee68d5a`. Remaining: 6 file-watcher tests + 1 proxy cancel. User updated title with root cause: `fs_watch` should watch parent dirs, not the file itself, for FSEvents to fire. Wants its own focused pass. |
| 16 | UAT-013 /settings dialog doesn't render visibly  | Partial                | yes (PTY-only)   | Backend wiring (commits `6ef1cc4`, `82833f8`, `810f6c0`) lands `default_provider` / `default_thinking_level` correctly in `setup.model` and `build_settings_entries`. Remaining: the live `/settings` overlay doesn't paint visibly in the user's PTY probe — needs render-path investigation. Probably an overlay lifecycle / repaint bug, not a value-lookup bug. |
| 19 | --export HTML shows everything as User role      | Not started            | unverified       | Likely in `crates/coding-agent/src/core/export.rs` HTML branch. Suspect a role mapping mistake (assistant blocks ending up under User). |
| 20 | --export --resume <session_id> "No session header found" | Not started    | unverified       | Wrong path resolution when combining `--export` with `--resume <id>`. Likely the export path uses the literal id as a filename instead of resolving it through `SessionManager` first. |
| 21 | README says settings.json, actual is settings.yaml | Not started          | yes (doc grep)   | One-line doc fix in `crates/coding-agent/README.md`. Quick win. |
