...
- A Gemini turn cut short by the token limit is no longer executed as if it were complete. The stop reason was overwritten with "the model asked for tools" whenever the turn contained a tool call, which disarmed the check that fails calls whose arguments were truncated — so the last call ran with whatever fragment had arrived. Safety and recitation stops were swallowed the same way ([#160](https://github.com/wanggang316/hand-ai/pull/160))
- Gemini 3 turns carrying more than one tool call get their results back on the right call. That generation pairs a result to its call by id rather than by position, and the ids were not being sent ([#155](https://github.com/wanggang316/hand-ai/pull/155))
- Claude models no longer lose a turn's reasoning when the provider delivers a thinking block whole instead of in pieces. The signature that arrives with it was dropped, and an unsigned thinking block is rejected on replay — costing the whole block, not just its provenance ([#157](https://github.com/wanggang316/hand-ai/pull/157))
- Copilot models that need their policy enabled at login actually get enabled. All of those requests went out at once, which tripped the provider's rate limiter; the rejected ones stayed disabled, and since this step is best-effort there was nothing to say so — the model was simply missing later ([#161](https://github.com/wanggang316/hand-ai/pull/161))
- DeepSeek V4 Flash offers its `low` reasoning effort. The level was marked unsupported for both native DeepSeek entries even though Flash accepts it, so the cheapest reasoning pass was unreachable on the model built for it ([#156](https://github.com/wanggang316/hand-ai/pull/156))
